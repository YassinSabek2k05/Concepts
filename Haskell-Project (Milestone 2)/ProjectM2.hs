...
-- a)
railsNetworkPrint x seed = printRailsNetwork (railsNetwork x seed)
railsNetwork x seed = diagonal0s (railsNetworkHelper (getRandomNumbers n seed) x) 0 where n = x*x
railsNetworkHelper l x = cut l x

cut [] i = []
cut l x = (take x l) : cut (drop x l) x

replaceIW0 (h:t) i | i==0 = 0:t
			   | otherwise = h:replaceIW0 t (i-1)
diagonal0s [] i = []
diagonal0s (h:t) i = replaceIW0 h i : diagonal0s t (i+1)

-- b)
pullLever seed r c ll = pullLeverHelper1 (head (getRandomNumbers 1 seed)) r c ll
pullLeverHelper1 value r c [] = error "invalid inputs"
pullLeverHelper1 value r c (h:t) | r/=(1) = h:(pullLeverHelper1 value (r-1) c t)
							 	 | otherwise = (pullLeverHelper2 value r c h):t

pullLeverHelper2 value r c [] = error "invalid inputs"
pullLeverHelper2 value r c (h:t) | c/=(1) = h:(pullLeverHelper2 value r (c-1) t)
								 | otherwise = value:t

pullLeverPrint seed r c ll = printRailsNetwork (pullLever seed r c ll)

-- c)
initializeSource n (h:t) = PQ (Node n 0) (initializeSourceHelper n (length h) 1)

initializeSourceHelper n size acc | acc == (size +1) = Empty
								  | n/=acc = PQ (Node acc 9999) (initializeSourceHelper n size (acc+1))
								  | otherwise = initializeSourceHelper n size (acc+1)

-- d)
-- converts a priorityqueue into a list of nodes
pqToList Empty = []
pqToList (PQ (Node a b) pqTail) = (Node a b):pqToList pqTail

-- sorts the list by distance distance
pqListSort [] = []
pqListSort (h:t) = pqListSortInsertion t [h]


getKey (Node a b) = b 

--inserts each node into the sorted list
pqListSortInsertion [] l = l
pqListSortInsertion (node:t) l = pqListSortInsertion t (slide node l)

-- inserts a node into the correct position in sorted way
slide n [] = [n]
slide (Node a b) [node] = if ((getKey node) < b) then [node,Node a b] else [Node a b, node]
slide (Node a b) (node:rest) =  if ((getKey node) < b) then node:(slide (Node a b) rest) else (Node a b):node:rest

listToPQ [] = Empty
listToPQ (h:t) = PQ h (listToPQ t)

updatePQ pq = listToPQ(pqListSort(pqToList pq))

-- retrieves the nth row from a 2d list (using 1 indxing)
getRow n [] = error "invalid index or the list is empty"
getRow 1 (h:t) = h
getRow row (h:t) = getRow (row-1) t

-- retrieves the nth element from a list (using 1 indexing )
getIndex n [] = error "invalid index or the list is empty"
getIndex 1 (h:t) = h
getIndex col (h:t) = getIndex (col-1) t

-- calculates the shortest path cost from a specific start node to a destination node
calculateShortestPathCost destinationNode network pq = dijkstra destinationNode network (updatePQ pq) []

-- dijkstra algorithm implementation to find shortest path cost
dijkstra _ _ Empty _ = 9999  -- if no path is found
dijkstra destinationNode network pq visited | nodeId == destinationNode = distance
                                            | elem nodeId visited = dijkstra destinationNode network rest visited
                                            | otherwise = dijkstra destinationNode network (updatePQ updatedQueue) (nodeId : visited)
  where
    (Node nodeId distance, rest) = extractMin pq
    neighbors = getNeighbors nodeId network
    updatedQueue = relaxEdges nodeId distance neighbors rest network

-- removes the node with the smallest distance from the priorityqueue
extractMin Empty = error "priorty queue is empty"
extractMin (PQ node rest) = (node, rest)

-- helper function to collect indexes of neighbors with weight thats not zero
getNeighborsHelper [] _ = []
getNeighborsHelper (x:xs) index | x > 0 = index : getNeighborsHelper xs (index + 1)
                                | otherwise = getNeighborsHelper xs (index + 1)

-- returns the list of neighboring node indexes for a given node
getNeighbors nodeId network = getNeighborsHelper (getRow nodeId network) 1

-- updates the queue by relaxing all edges from the current node
relaxEdges _ _ [] pq _ = pq
relaxEdges nodeId distance (neighbor:rest) pq network = relaxEdges nodeId distance rest newPQ network
  where
    edgeWeight = getIndex neighbor (getRow nodeId network)
    newDistance = distance + edgeWeight
    newPQ = updateNodeDistance neighbor newDistance pq

-- updates the distance of a node in the priorityqueue if shorter path is found
updateNodeDistance _ _ Empty = Empty
updateNodeDistance targetId newDistance (PQ (Node nodeId distance) rest) | (nodeId == targetId && newDistance < distance )= PQ (Node nodeId newDistance) rest
                                                                         | otherwise = PQ (Node nodeId distance) (updateNodeDistance targetId newDistance rest)

-- generates a list of x random numbers between 0 and 9 
getRandomNumbers x seed = take x (randomRs (0, 9) (mkStdGen seed))


-- prints a 2d list to the console 
...
-------------Begin calculateShortestPathCost Tests--------------

test16 = TestCase(assertEqual "public16" 0 (calculateShortestPathCost 4 (railsNetwork 5 12345) (initializeSource 4 (railsNetwork 5 12345))))

test17 = TestCase(assertEqual "public17" 6 (calculateShortestPathCost 9 (railsNetwork 10 9384573) (initializeSource 1 (railsNetwork 10 9384573))))

test18 = TestCase(assertEqual "public18" 4 (calculateShortestPathCost 15 (railsNetwork 15 347) (initializeSource 6 (railsNetwork 15 347))))

test19 = TestCase(assertEqual "public19" 3 (calculateShortestPathCost 2 (railsNetwork 20 4588544) (initializeSource 15 (railsNetwork 20 4588544))))

test20 = TestCase(assertEqual "public20" 2 (calculateShortestPathCost 20 (railsNetwork 25 92741563) (initializeSource 25 (railsNetwork 25 92741563))))

--------------End calculateShortestPathCost Tests---------------
